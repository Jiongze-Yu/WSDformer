...
for i=1:1200                         % the number of testing samples DID-Data
   x_true=im2double(imread(strcat('..\..\datasets\DID-Data\test\gt\',sprintf('%d.jpg',i))));  % groundtruth 
   x_true = rgb2ycbcr(x_true);
   x_true = x_true(:,:,1); 
   x = im2double(imread(strcat('..\..\results\WSDformer_DID-Data\',sprintf('%d.jpg',i))));     %reconstructed image
   x = rgb2ycbcr(x);
   x = x(:,:,1);
   tp= tp+ psnr(x,x_true);
